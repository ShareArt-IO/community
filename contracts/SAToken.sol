...
contract LockableTokenImpl {
    ...
    modifier canTransfer(address _from, uint256 _value) {
        if ( lockBalances[_from].releaseAmountPerMonth == 0) {
			...
			if( round > lockBalances[_from].limitRound){
				_;
			} 	else {
				require(round >= lockBalances[_from].round && round > 0 );
				...
				_;
				 lockBalances[_from].allowance =  lockBalances[_from].allowance.sub(_value);
			 }
        }
    }

    function lock(address _addr,uint256 _percent,uint8 _limitRound,address _whereLock) internal {
        ...
        if (lockBalances[_addr].balance > 0) {
            ...
            lockBalances[_addr].limitRound= _limitRound;
        }
    }
    
    function addSupply(address _addr,uint256 _amount) internal {
        lockBalances[_addr] = LockInfo(_amount, 0, 0, 1, 0,0);
    }
    ...
    function changeBalanceDo(address _from,address _to) internal returns(bool) {
        ...
        lockBalances[_from] = LockInfo(0, 0, 0, 0, 0, 0);
    }
}
...
contract SAToken is Ownable, Token , LockableTokenImpl {
    ...
    function SAToken(
        ...
        */
        ...
        investorAddr=_investor;
        round = 0;
    }
    
     function endCrawl() public onlyOwner {
		 require(fundAddr !=address(0));
		 transferDo(investorAddr,fundAddr, getBalance (investorAddr));
	 }
	 
    function unlockThisRound(uint8 _round) public onlyOwner returns(bool) {
			if(_round == 0){
				round = round + 1;
			} else {
				round = _round;
    ...
    function lockTransfer(address _to,uint256 _percent,uint8 _limitRound) public {
		require( investorAddr == msg.sender);
        lock(_to,_percent,_limitRound,msg.sender);
    }
